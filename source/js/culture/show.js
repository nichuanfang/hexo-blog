$(document).ready(function () {
    ...
    if (document.querySelector('.show-culture-list')) {
        ...
        var jsonSrc = 'https://api.jaychou.site/trakt/show'

        var coverSrc = 'https://image.tmdb.org/t/p/w116_and_h174_face'

        var init_data = ''
        // 如果SessionStorage中有init_data，则读取init_data
        if (sessionStorage.getItem('show_init_data')) {
            init_data = sessionStorage.getItem('show_init_data')
        ...
        data_ended = false
        // 获取元素.culture-list
        var cultureList = document.querySelector('.show-culture-list')
        ...
        function generateShowElements(data, coverSrc) {
            // 假设你的 JSON 数据是一个包含多个剧集的数组，每个剧集对象都有相应的属性
            var jsonData = data['data']['data']

            // 获取目标容器元素
            var container = document.querySelector('.show-culture-list')
            ...
            jsonData.forEach(function (item) {
                ...
                // 创建超链接 url为剧集详情页  剧集id为参数

                var url = '/culture/shows/detail/?tmdb_id=' + item.show_id
